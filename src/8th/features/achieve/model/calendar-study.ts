...
function makeCalendarStudy(json: any): CalendarStudy {
  return {
...
function transform(json: any): CalendarStudyResponse {
  return {
    beforeSetup: makeCalendarStudy(json?.BeforeSetup),
    list: json?.StudyCalendar?.map((item: any) => makeCalendarStudy(item)),
  }
}

export type CalendarStudyParams = {
  year: number
  month: number
}

export type CalendarStudyResponse = {
  beforeSetup: CalendarStudy
  list: CalendarStudy[]
}

export async function getCalendarStudy(
  input: CalendarStudyParams,
): Promise<CalendarStudyResponse> {
  const request = makeRequest(`api/calendar/study`, {
    method: 'get',
    queryString: {
      year: input.year,
      month: input.month,
