...
import CalendarHeaderDnf from '../component/CalendarHeaderDnf'
import CalendarItemDnf from '../component/CalendarItemDnf'

/**
 * 도도앤프렌즈 전용 캘린더 모달 (기존 CalendarModal 복사)
 */
...
interface CalendarModalDnfProps {
  onCloseModal: () => void
}

export default function CalendarModalDnf({
  onCloseModal,
}: CalendarModalDnfProps) {
  // @Language 'common'
  ...
  const onCalendarItemClick = (year: number, month: number, day: number) => {
    ...
    router.push(
      `${SITE_PATH.DODON_FRIENDS.REVIEW}?startDate=${date}&endDate=${date}`,
    )
    if (onCloseModal) {
      onCloseModal()
    }
