...
export function makeCustomer(json?: any): Customer {
  return {
    ...
    preKMode: RenewType.renewString(json?.PreKMode),
  }
}

export function parseCustomer(json?: string): {
  Token: string
  Customer: Customer
} {
  if (!json) {
  ...
  return {
    Token: RenewType.renewString(obj?.Token),
    Customer: makeCustomer(obj?.Customer),
  }
}
