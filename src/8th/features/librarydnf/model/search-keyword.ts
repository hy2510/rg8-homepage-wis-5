import { isKeywordSearchLevelAllowed } from '@/8th/features/librarydnf/utils/book-level'
...
function transform(json: any): SearchBookKeywordResponse {
  const books = json.Books.map((item: any) => transformSearchBook(item)).filter(
    (book: SearchBook) => isKeywordSearchLevelAllowed(book.levelName),
  )
