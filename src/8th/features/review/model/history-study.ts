...
function makeHistoryStudy(json?: any): HistoryStudy {
  return {
...
function transform(json: any): HistoryStudyResponse {
  return {
    history: json?.History?.map((item: any) => makeHistoryStudy(item)),
    summary: {
      totalCount: RenewType.renewNumber(json?.Summary?.TotalCount),
      passCount: RenewType.renewNumber(json?.Summary?.PassedCount),
      failCount: RenewType.renewNumber(json?.Summary?.FailedCount),
      studyDays: RenewType.renewNumber(json?.Summary?.StudyDays),
      totalPoints: RenewType.renewNumber(json?.Summary?.TotalPoints),
    },
    page: transformPagination(json?.Pagination),
    download: RenewType.renewString(json?.ExcelDownload),
    performanceReport: RenewType.renewString(json?.PerformanceReport),
  }
}

export type HistoryStudyParams = {
  startDate?: string
  endDate?: string
  keyword?: string
  status: string
...
export type HistoryStudyResponse = {
  history: HistoryStudy[]
  summary: {
    totalCount: number
    passCount: number
    failCount: number
...
export async function getHistoryStudy(
  input: HistoryStudyParams,
): Promise<HistoryStudyResponse> {
