...
export default function BoardDetailTypeNotice({
  backLabel,
  backLink,
  title,
  date,
  ...
  return (
    <main className="container compact">
      ...
      <div className={style.rg_news_post}>
        ...
        {file && fileName && (
          <AttachmentFileDownload file={fileName} link={file} />
        )}

        <div className={style.row_2}>
          <HtmlViewer htmlContents={htmlContents} />
          {showImage && (
            <div>
              <img
                src={image}
...
function HtmlViewer({ htmlContents }: { htmlContents: string }) {
  const router = useRouter()
  ...
  const onOpenLink = (
    ...
    if (previewRef.current) {
      const anchorTag = findAnchorTag(previewRef.current, target)
      if (anchorTag !== null) {
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        onOpenLink(e.target as Element | null, e)
      }}
      onAuxClick={(e) => {
        if (e.button === 2) {
          return
