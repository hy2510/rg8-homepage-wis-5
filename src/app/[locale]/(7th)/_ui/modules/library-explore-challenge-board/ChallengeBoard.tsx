...
import { EventPrize } from '@/7th/_repository/client/object/event-prize'
import { AlertBar, Modal } from '@/7th/_ui/common/common-components'
...
export function ChallengeBoard({
  ...
  const progressDay = Math.min(
    eventDay - DateUtils.dayDistance(todayObj, endDateObj, false),
    eventDay,
  )
  ...
  return (
    <Modal
      ...
      <div className={style.set_my_goal}>
        <div>
          <span style={{ color: 'var(--gray1)', fontSize: 'var(--text-m)' }}>
            {` * ${t('t931')}`}
          </span>
        </div>
        ...
        <div className={style.row_a}>
          ...
          <div>
            ...
            <span style={{ marginLeft: '5px' }}>
              {`${NumberUtils.getHundredPercentage(currentStudyDays, finalStudyDays)}%`}
            </span>
          </div>
        </div>
    ...
    <div className={style.challenge_progress}>
      <div className={style.progress_info}>
        ...
        <div className={style.row_b}>
          ...
          <div className={style.txt_d}>
            D-
            {`${eventDay - progressDay === 0 ? 'Day' : `${eventDay - progressDay}`}`}
          </div>
          {!isDropout &&
